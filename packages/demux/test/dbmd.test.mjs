...
function dbmd(modes) {
  const payload = new Uint8Array(7 + 6 * 15 + modes.length * 2);
  const view = new DataView(payload.buffer);
  view.setUint32(0, 0xf8726fbd, true);
  view.setUint16(4, modes.length, true);
  payload.set(modes.map((mode) => ({ off: 0, near: 1, far: 2, mid: 3, "not-indicated": 4 })[mode]), 7 + 6 * 15 + modes.length);
...
assert.deepEqual(metadata.modeTable, ["off", "near", "far", "mid", "not-indicated"]);
assert.equal(metadata.elementMapping, "unavailable");
...
assert.deepEqual(discovered.modeTable, ["near"]);
assert.equal(discovered.elementMapping, "unavailable");
