...
const hrtfDir = process.argv[3] ?? "apps/web/public/hrtf";

// ---- 真实内容逐布局 L/R 平衡 ----
// 1) 全场（相干卷积和）L/R 能量
// 2) 逐总线贡献：bus 信号能量 × bus IR 耳能量（非相干近似）→ 定位倾斜来源
console.log("\n=== 真实内容（30–120s）逐布局 L/R ===");
for (const layoutId of ["5.1.4", "7.1.4", "9.1.4"]) {
  ...
  for (let bus = 0; bus < layout.length; bus++) {
    if (layout[bus].isLfe) continue;
    const ir = loadIr(hrtfDir, manifest, layout[bus], 0.04);
    const busOut = [new Float64Array(captureLength + 8191), new Float64Array(captureLength + 8191)];
    convolveBus(buses[bus], ir, busOut);
    const sigE = buses[bus].reduce((s, v) => s + v * v, 0);
    const eL = busOut[0].reduce((s, v) => s + v * v, 0);
    const eR = busOut[1].reduce((s, v) => s + v * v, 0);
    busRows.push({
      bus: layout[bus].name,
      az: layout[bus].azimuth,
      sigDb: +(10 * Math.log10(sigE + 1e-18)).toFixed(1),
      lrDb: +(20 * Math.log10(Math.sqrt((eL + 1e-18) / (eR + 1e-18)))).toFixed(2),
    });
    for (let i = 0; i < outputs[0].length; i++) {
      outputs[0][i] += busOut[0][i];
      outputs[1][i] += busOut[1][i];
    }
  }
  const trimmed = outputs.map((o) => o.subarray(preRoll, preRoll + (endSample - startSample)));
  const eL = trimmed[0].reduce((s, v) => s + v * v, 0);
  const eR = trimmed[1].reduce((s, v) => s + v * v, 0);
  console.log(`\n${layoutId}: 全场 L/R = ${(20 * Math.log10(Math.sqrt(eL / eR))).toFixed(3)}dB`);
  console.table(busRows);
  // 10s 窗 L/R 轨迹
  const win = 10 * 48000;
  const trajectory = [];
  for (let from = 0; from + win <= trimmed[0].length; from += win) {
    let wl = 0, wr = 0;
    for (let i = from; i < from + win; i++) { wl += trimmed[0][i] ** 2; wr += trimmed[1][i] ** 2; }
    trajectory.push(+(20 * Math.log10(Math.sqrt(wl / wr))).toFixed(2));
  }
  console.log(`${layoutId} 10s窗 L/R 轨迹: ${trajectory.join(" ")}`);
}
